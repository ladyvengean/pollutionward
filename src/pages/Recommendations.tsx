...
function WardRecommendationSection() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/api/attribution?ward_id=${encodeURIComponent(
          ward
        )}`
      );
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-6 shadow-card">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Enter ward ID (e.g. DELHI_001) to generate real-time recommendations
      </p>
      ...
      {data && (
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-2xl font-bold">{data.ward}</h3>
              <p className="text-sm text-muted-foreground">
                Station: {data.station_name} ({data.station_distance_km} km)
              </p>
            </div>
            <div className="text-right">
              <Badge variant="aqiSevere">{data.aqi_category}</Badge>
          ...
          {/* POLLUTANTS */}
          <div>
            <h4 className="font-semibold mb-2">Pollutant Levels</h4>
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
              <Pollutant label="PM2.5" value={data.pollutants.pm25} />
              <Pollutant label="PM10" value={data.pollutants.pm10} />
              <Pollutant label="NO₂" value={data.pollutants.no2} />
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <h1 className="text-3xl font-bold">
          Actionable Ward Recommendations
        </h1>
